...
@Configuration
@ConditionalOnProperty(name = "springdoc.show-actuator.dirty-fix-enabled", havingValue = "true")
public class DirtyFixConfig {

  /**
   * Because of the new Actuator implementation in Spring Boot 2,
   * all actuator endpoints are now dynamically mapped to a single handler method:
   ...
   * <ul>
   *   <li>
   *     Because the handler method has an @RequestBody annotated 'body' parameter,
   *     this parameter appears in all actuator endpoints as body parameter,
   *     even for GET and HEAD requests (which cannot have a request body).
   *     These endpoints cannot be executed from the Swagger UI page.
   *   </li>
   *   <li>
   *     If an Actuator endpoint contains path parameters,
   *     these are not available as input fields on the Swagger UI page,
